...
"""
=========================================================
💎 ALMASI QUANT v183 - Live Hardened & Debug Edition
Fixed: Symbol, Quality Filter, Robustness, Logging, Live Safety
=========================================================
"""
...
class Engine:
    def __init__(self):
        ...
        TG.send(f"🛡 <b>Almasi v183 Live</b>\nDRY_RUN: {DRY_RUN} | Strict: {STRICT_QUALITY} | Symbols: {len(SYMBOLS)}")
    ...
    def _scan(self, bal):
        now = time.time()
        log.info(f"SCAN | Bal: ${bal:.2f} | Open: {len(self._pos)}")
        for sym in SYMBOLS:
    ...
    def _execute_trade(self, sym, side, sc, exps, px, atr, bal):
        ...
        try:
            ...
            if o:
                ...
                pos = {"id":pid,"symbol":sym,"side":side,"entry":fpx,"qty":qty,"sl":real_sl,"tp":real_tp,
                       "reason":f"Score {sc} [{', '.join(exps[:3])}]", "conf": 75 + abs(sc)*8, "atr":atr}
                with self._lock: self._pos[pid] = pos
                TR.init(pid, fpx, real_sl)
                database.insert(pos)
                self._last_signal[sym] = int(time.time())
                TG.send(f"🟢 <b>OPEN {side.upper()}</b> {sym}\nEntry: {fpx:.4f}\nQty: {qty} | Reason: {pos['reason']}")
        finally:
            with self._lock:
    ...
    def _exits(self):
        ...
        for pid, pos in snap.items():
            ...
            if not px: continue
            current_atr = pos.get("atr", abs(pos["entry"] - pos["sl"]) / 1.5)
            ...
            if (pos["side"]=="long" and px >= pos["tp"]) or (pos["side"]=="short" and px <= pos["tp"]):
                self._close(pid, pos, px, "Take Profit")
            elif (pos["side"]=="long" and px <= nsl) or (pos["side"]=="short" and px >= nsl):
                self._close(pid, pos, px, "Trailing Stop")
    ...
    def _close(self, pid, pos, px, reason):
        ...
        TR.rm(pid)
        self._cooldown[active_pos["symbol"]] = time.time()
        ...
        fees = (active_pos["entry"] * active_pos["qty"] * TAKER_FEE) + (px * active_pos["qty"] * TAKER_FEE) if DRY_RUN else 0
        net_pnl = gross_pnl - fees
        pct = (net_pnl / (active_pos["entry"] * active_pos["qty"])) * LEVERAGE * 100 if active_pos["entry"] * active_pos["qty"] != 0 else 0
        
        database.close(pid, px, net_pnl, pct, reason)
        TG.send(f"{'💸' if net_pnl>0 else '🩸'} <b>CLOSE</b> {active_pos['symbol']}\n{reason}\nNet: {net_pnl:+.2f}$ ({pct:+.2f}%)")
...
HTML = """<!DOCTYPE html><html dir="ltr" lang="fa"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Almasi v183</title><style>body{font-family:'Segoe UI',Tahoma,sans-serif;background:#0d1117;color:#c9d1d9;padding:15px;margin:0;direction:ltr;}/* بقیه استایل همان قبلی */</style></head><body><h1 style="text-align:center">Almasi Quant v183 Live</h1><!-- بقیه HTML همان نسخه قبلی را کپی کنید --></body></html>"""

@app.route('/')
def home():
    if not engine: return "Warming up...", 503
    with engine._lock: pos_list = list(engine._pos.values())
    # ... (بقیه داشبورد همان قبلی - برای اختصار)
    return render_template_string(HTML, dry=DRY_RUN, lev=LEVERAGE, tf=TF, bal=EX.balance(), today=database.today(), 
                                  open_pos=len(pos_list), max_pos=MAX_POS, radar=engine.radar, positions=[], history=database.recent_closed(7))
