...
# -*- coding: utf-8 -*-
"""
Master Quant Engine v9.4 (Notional Value Patch & Syntax Fix)
- Fixes TE_ORDER_VALUE_TOO_SMALL
- Flawless Trailing, Syncing, and Partial TP intact.
- PEP-8 Compliant (No squished lines).
"""
...
MAX_POS = 4
MAX_DD = 10.0  
MIN_ORDER_USD = 15.0 # Minimum Order Value to prevent exchange reject
...
class QuantEngine:
    ...
    async def execute_trade(self, sym: str, sig: Dict):
        ...
        if not price or SHARED_STATE["balance"] < 5.0: 
            return 
        ...
        # Ensures order value is at least MIN_ORDER_USD ($15)
        raw_qty = risk_amount / dist
        if (raw_qty * price) < MIN_ORDER_USD:
            raw_qty = MIN_ORDER_USD / price

        try: 
            qty = float(self.ex.amount_to_precision(sym, raw_qty))
        except: 
            qty = round(raw_qty, 3)
    ...
    async def run_live_test(self):
        await self.tg.send("🧪 <b>Initiating 30-Second Live Test...</b>\nForcing trades at minimum required value.")
        try:
            # Force exactly $15 worth of BTC/ETH to bypass Phemex TE_ORDER_VALUE_TOO_SMALL limit
            btc_price = self.prices.get("BTC/USDT", 60000)
            eth_price = self.prices.get("ETH/USDT", 3000)
            
            btc_qty = MIN_ORDER_USD / btc_price
            eth_qty = MIN_ORDER_USD / eth_price
            
            try: btc_qty = float(self.ex.amount_to_precision("BTC/USDT", btc_qty))
            except: btc_qty = round(btc_qty, 3)
            
            try: eth_qty = float(self.ex.amount_to_precision("ETH/USDT", eth_qty))
            except: eth_qty = round(eth_qty, 3)
