...
# -*- coding: utf-8 -*-
"""
Master Quant Engine v9.4 (Notional Value Patch)
- Fixes TE_ORDER_VALUE_TOO_SMALL (Forces min $15 order value).
- Flawless Trailing, Syncing, and Partial TP intact.
"""
...
MAX_POS = 4
MAX_DD = 10.0  
MIN_ORDER_USD = 15.0 # 🔴 FIX: Minimum Order Value to prevent exchange reject
...
log = logging.getLogger("QuantV9.4")
...
class AsyncDB:
    def __init__(self, db_path="bot_v9.db"): self.db_path = db_path
    ...
    async def update_analytics(self):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT pnl FROM trades WHERE status='closed'") as cursor:
                rows = await cursor.fetchall(); if not rows: return
...
class StrategyEngine:
    def analyze(self, df: pd.DataFrame) -> Dict:
        ...
        c = df_c['close']; tr = pd.concat([df_c['high'] - df_c['low'], (df_c['high'] - c.shift()).abs(), (df_c['low'] - c.shift()).abs()], axis=1).max(axis=1)
        atr = tr.ewm(com=13, adjust=False).mean().iloc[-1]; price = c.iloc[-1]
        delta = c.diff(); up = delta.clip(lower=0); down = -1 * delta.clip(upper=0)
        rs = up.ewm(com=13, adjust=False).mean() / down.ewm(com=13, adjust=False).mean(); rsi = 100 - (100 / (1 + rs))
        ema20 = c.ewm(span=20).mean(); ema50 = c.ewm(span=50).mean()
        
        sig = {"action": "neutral"}
        if price > ema20.iloc[-1] > ema50.iloc[-1] and rsi.iloc[-1] < 45: sig = {"action": "buy", "strat": "Pullback_Long"}
        elif price < ema20.iloc[-1] < ema50.iloc[-1] and rsi.iloc[-1] > 55: sig = {"action": "sell", "strat": "Pullback_Short"}
...
class AsyncTelegram:
    def __init__(self, engine):
        self.engine = engine; self.base_url = f"https://api.telegram.org/bot{TG_TOKEN}"; self.offset = 0
    def main_menu(self):
    ...
    def close_menu(self, pid): return {"inline_keyboard": [[{"text": "❌ Force Close", "callback_data": f"close_{pid}"}]]}
    async def send(self, msg: str, reply_markup=None):
        if not TG_TOKEN: return
        payload = {"chat_id": TG_CHAT, "text": msg, "parse_mode": "HTML"}
        if reply_markup: payload["reply_markup"] = reply_markup
        try: async with aiohttp.ClientSession() as s: await s.post(f"{self.base_url}/sendMessage", json=payload)
        except: pass
    async def answer_callback(self, cb_id: str, text: str):
        try: async with aiohttp.ClientSession() as s: await s.post(f"{self.base_url}/answerCallbackQuery", json={"callback_query_id": cb_id, "text": text})
        except: pass
    async def poll(self):
        if not TG_TOKEN: return
        mode = "TESTNET" if TESTNET else "MAINNET (Real Money)"
        await self.send(f"🚀 <b>Master Quant V9.4 Online</b>\nNetwork: <b>{mode}</b>\nSelect an option below:", self.main_menu())
        while True:
            try:
                async with aiohttp.ClientSession() as s:
                    async with s.get(f"{self.base_url}/getUpdates?offset={self.offset+1}&timeout=10") as r:
                        ...
                        for upd in data.get("result", []):
                            ...
                            if "message" in upd:
                                if upd["message"].get("text", "") in ("/start", "/menu"): await self.send("🎛 <b>Main Control Panel</b>", self.main_menu())
                            if "callback_query" in upd:
                                cb = upd["callback_query"]; cb_id = cb["id"]; data = cb["data"]
                                await self.answer_callback(cb_id, "Processing...")
                                if data == "cmd_start": SHARED_STATE["is_active"] = True; await self.send("▶️ <b>Bot Started Scanning.</b>", self.main_menu())
                                elif data == "cmd_pause": SHARED_STATE["is_active"] = False; await self.send("⏹ <b>Bot Paused.</b>", self.main_menu())
                                elif data == "cmd_sync": await self.send("🔄 Force Syncing..."); await self.engine.smart_sync_positions()
                                elif data == "cmd_dash": await self.send(f"📊 <b>Pro Dashboard</b>\nState: {'🟢 Running' if SHARED_STATE['is_active'] else '🔴 Paused'}\nBalance: <b>${SHARED_STATE['balance']:.2f}</b>\nTotal PnL: <b>${SHARED_STATE['stats']['total_pnl']:.2f}</b>\nActive Pos: {len(SHARED_STATE['active_positions'])}/{MAX_POS}", self.main_menu())
                                elif data == "cmd_pos":
                                ...
                                elif data.startswith("close_"): await self.engine.force_close_position(data.split("close_")[1], "Closed via Telegram")
                                elif data == "cmd_livetest": asyncio.create_task(self.engine.run_live_test())
            except: pass
            await asyncio.sleep(1)

class QuantEngine:
    def __init__(self):
        self.db = AsyncDB(); self.strategy = StrategyEngine(); self.tg = AsyncTelegram(self)
        self.ex = ccxt.phemex({'apiKey': API_KEY, 'secret': API_SECRET, 'enableRateLimit': True, 'options': {'defaultType': 'swap'}})
        self.ex.set_sandbox_mode(TESTNET); self.prices = {}; self.markets_cache = {}; self.loop_count = 0
    async def start(self):
        await self.db.init_db(); await self.db.update_analytics()
        try:
        ...
        for t in await self.db.get_open_trades(): SHARED_STATE["active_positions"][t['id']] = t
        ...
        while True:
            try:
                ...
                if SHARED_STATE["peak_balance"] > 0:
                    ...
                    if dd >= MAX_DD and not SHARED_STATE["dd_halted"]: SHARED_STATE["dd_halted"] = True; await self.tg.send(f"🛑 <b>HALTED</b>\nDrawdown: {dd:.1f}%")
                    elif dd < MAX_DD * 0.8 and SHARED_STATE["dd_halted"]: SHARED_STATE["dd_halted"] = False
            except: pass
    ...
    async def execute_trade(self, sym: str, sig: Dict):
        ...
        dist = abs(price - sig['sl']); if dist == 0: return
        
        # 🔴 FIX: Ensure order value is at least MIN_ORDER_USD ($15)
        raw_qty = risk_amount / dist
        if (raw_qty * price) < MIN_ORDER_USD:
            raw_qty = MIN_ORDER_USD / price

        try: qty = float(self.ex.amount_to_precision(sym, raw_qty))
        except: qty = round(raw_qty, 3)
        ...
        try:
            ...
            SHARED_STATE["active_positions"][pid] = pos; await self.db.insert_trade(pos)
    ...
    async def run_live_test(self):
        await self.tg.send("🧪 <b>Initiating 30-Second Live Test...</b>\nForcing 1 Long (BTC) and 1 Short (ETH) at ~$15 Value.")
        try:
            # 🔴 FIX: Force exactly $15 worth of BTC/ETH to bypass limit
            btc_price = self.prices.get("BTC/USDT", 60000); eth_price = self.prices.get("ETH/USDT", 3000)
            btc_qty = float(self.ex.amount_to_precision("BTC/USDT", MIN_ORDER_USD / btc_price))
            eth_qty = float(self.ex.amount_to_precision("ETH/USDT", MIN_ORDER_USD / eth_price))
            ...
            await self.force_close_position(btc_pid, "End of Live Test"); await self.force_close_position(eth_pid, "End of Live Test")
            await self.tg.send("🎉 <b>Live Test Successfully Completed!</b>")
        except Exception as e: await self.tg.send(f"❌ <b>Live Test Failed:</b>\n{str(e)}")
    ...
    async def watchdog_loop(self):
        while True:
            for pid, pos in list(SHARED_STATE["active_positions"].items()):
                ...
                if sl_hit or tp_hit: await self.force_close_position(pid, "SL/Trailing" if sl_hit else "TP")
            await asyncio.sleep(1)

    async def force_close_position(self, pid: str, reason: str):
        pos = SHARED_STATE["active_positions"].get(pid); if not pos: return
        price = self.prices.get(pos['symbol'], pos['entry']); close_side = 'sell' if pos['side'] == 'buy' else 'buy'
        try:
            ...
            del SHARED_STATE["active_positions"][pid]; await self.db.update_analytics()
            await self.tg.send(f"{'🟢' if pnl >= 0 else '🔴'} <b>Closed ({reason})</b>\n{pos['symbol']} | PnL: ${pnl:.2f}")
        ...
        try:
            ...
            for p in remote:
                ...
                if size > 0:
                    ...
                    if not any(pos['symbol'] == matched_sym for pos in SHARED_STATE["active_positions"].values()):
                        ...
                        entry = float(p.get('entryPrice', 0)); side = 'buy' if p.get('side') == 'long' else 'sell'; price = self.prices.get(matched_sym, entry)
                        ...
                        SHARED_STATE["active_positions"][pid] = pos_data; await self.db.insert_trade(pos_data)
            ...
            for pid, pos in list(SHARED_STATE["active_positions"].items()):
                if pos['symbol'] not in active_remote_syms and pos['strategy'] != 'LiveTest':
                    await self.db.close_trade(pid, 0.0); del SHARED_STATE["active_positions"][pid]
