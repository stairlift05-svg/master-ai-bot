...
# -*- coding: utf-8 -*-
"""
Master Quant Engine v9.2 (Pro Telegram & Sync Fix)
- Ironclad Web Security (Forced Basic Auth).
- Aggressive Phemex Position Sync (Finds manual trades).
- Pro Telegram UI (Inline Glass Buttons & Callback Queries).
"""
...
# ============================================================================
# 1. CONFIGURATION (With Fallback Security)
# ============================================================================
load_dotenv()
...
log = logging.getLogger("QuantV9.2")
...
class StrategyEngine:
    def analyze(self, df: pd.DataFrame) -> Dict:
        ...
        c = df_c['close']; atr = Indicators.atr(df_c).iloc[-1]; price = c.iloc[-1]; rsi = Indicators.rsi(c)
...
# ============================================================================
# 4. PRO ASYNC TELEGRAM (Inline Keyboards)
# ============================================================================
class AsyncTelegram:
    ...
    async def poll(self):
        if not TG_TOKEN: return
        # Send greeting menu
        await self.send("🚀 <b>Master Quant V9.2 Online</b>\nSelect an option below:", self.main_menu())
                ...
                async with aiohttp.ClientSession() as s:
                    async with s.get(f"{self.base_url}/getUpdates?offset={self.offset+1}&timeout=10") as r:
                        ...
                        for upd in data.get("result", []):
                            ...
                            # 🔴 FIX: Handle Inline Button Clicks
                            if "callback_query" in upd:
                                ...
                                if data == "cmd_start":
                                    ...
                                    await self.send("⏹ <b>Bot Paused (Watchdog still active).</b>", self.main_menu())
                                elif data == "cmd_sync":
                                    ...
                                    msg = f"📊 <b>Pro Dashboard</b>\n\nState: {st}\nBalance: <b>${b:.2f}</b>\nTotal PnL: <b>${p:.2f}</b>\nActive Pos: {len(SHARED_STATE['active_positions'])}/{MAX_POS}"
                                    ...
                                    if not pos:
                                        ...
                                        for pid, p in pos.items():
                                            ...
                                            ico = "🟩" if pnl > 0 else "🟥"
                                            msg = f"{ico} <b>{p['symbol']}</b> ({p['side'].upper()})\nEntry: {p['entry']:.4f}\nPnL: ${pnl:.2f}"
                                            await self.send(msg, self.close_menu(pid))
...
class QuantEngine:
    ...
    async def start(self):
        ...
        # 🔴 FIX: Force immediate sync on boot to catch manual trades!
    ...
    async def smart_sync_positions(self):
        """ 🔴 FIX: Aggressive Phemex Parse """
        try:
            remote = await self.ex.fetch_positions(SYMBOLS)
            active_remote_syms = []
            
            for p in remote:
                # Handle Phemex weird JSON structure
                raw_size = p.get('contracts', 0)
                if not raw_size: raw_size = p.get('info', {}).get('size', 0)
...
# ============================================================================
# 6. WEB DASHBOARD (Strict Lockdown Auth)
# ============================================================================
app = Flask(__name__)
auth = HTTPBasicAuth()
...
def dashboard():
    html = """
    <!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Quant V9.2 Pro Dashboard</title>
    <style>
    ...
    <script>
        async function update() {
            ...
            if(Object.keys(d.active_positions).length === 0) pDiv.innerHTML = "<span style='color:#8b949e'>No active positions.</span>";
        } setInterval(update, 2000); window.onload = update;
    </script></head><body>
    <h1 style="text-align:center">🤖 Master Quant Engine V9.2</h1>
    <div class="grid">
        ...
        <div class="card"><h2>Stats</h2><p>Total PnL: <span id="pnl" class="val"></span></p></div>
...
if __name__ == "__main__":
    ...
    except KeyboardInterrupt: log.info("🛑 Shutting down V9.2...")
