...
# -*- coding: utf-8 -*-
"""
Master-AI Quant Bot v6.3
- Phemex Testnet با فرمت صحیح نمادها
- رفع مشکل BTC/USDT vs BTC/USDT:USDT
"""
...
log = logging.getLogger("MasterQuant_v6.3")
...
# ─── نمادهای مورد نظر (فرمت استاندارد - ربات خودش تبدیل می‌کند) ───
...
# ============================================================================
# SYMBOL RESOLVER - قلب اصلاح
# ============================================================================
class SymbolResolver:
    """
    تبدیل هوشمند نمادها:
    BTC/USDT → پیدا کردن فرمت صحیح در صرافی
    (BTC/USDT یا BTC/USDT:USDT یا BTCUSDT و ...)
    """

    def __init__(self):
        self._map: Dict[str, str] = {}      # wanted → real
        self._rev: Dict[str, str] = {}      # real → wanted
        self._active: List[str]   = []      # لیست نمادهای فعال (real)

    def resolve(self, markets: Dict, wanted: List[str]) -> None:
        """
        بررسی هر نماد خواسته‌شده و پیدا کردن معادل واقعی در صرافی
        """
        self._map.clear()
        self._rev.clear()
        self._active.clear()

        log.info("🔍 بررسی نمادها در صرافی...")
        ...
        for sym in wanted:
            ...
            if not found:
                # جستجوی فازی
                for mk in markets:
                    if base in mk and quote in mk:
                        mtype = markets[mk].get("type","")
                        if mtype in ("swap","future","linear",""):
                            found = mk
                            break
            ...
            if found:
                self._map[sym]   = found
                self._rev[found] = sym
                self._active.append(found)
                if found != sym:
                    log.info("  ✅ %s → %s", sym, found)
                else:
                    log.info("  ✅ %s", sym)
            else:
                log.warning("  ❌ %s → یافت نشد!", sym)

        log.info("📊 %d/%d نماد فعال", len(self._active), len(wanted))

    def real(self, wanted: str) -> str:
        """برگرداندن نام واقعی نماد در صرافی"""
        return self._map.get(wanted, wanted)

    def wanted(self, real: str) -> str:
        """برگرداندن نام خواسته‌شده از نام واقعی"""
        return self._rev.get(real, real)

    def base(self, sym: str) -> str:
        """BTC/USDT → BTC"""
        return sym.split("/")[0]

    @property
    def active_wanted(self) -> List[str]:
        """لیست نمادهای فعال به فرمت خواسته‌شده"""
        return list(self._map.keys())

    @property
    def active_real(self) -> List[str]:
        """لیست نمادهای فعال به فرمت واقعی"""
        return list(self._active)

    def get_contract_size(self, sym: str) -> float:
        base = self.base(sym)
        return CONTRACT_SIZE_MAP.get(base, 0.001)
...
class SelfDiagAI:
    ...
    def _chk_ohlcv_errors(self) -> List[DiagIssue]:
        now  = time.time()
        errs = [e for e in self._err_hist
        ...
        return [DiagIssue(
            ...
            recommendation=(
                "۱. صفحه /diagnose را باز کنید\n"
                "۲. PHEMEX_TESTNET را بررسی کنید\n"
                "۳. فرمت نمادها را بررسی کنید"
            ),
            data={"count": len(errs), "by_sym": dict(by_sym)},
        )]
    ...
    def _chk_system(self, exchange, engine) -> Dict:
        ...
        if not exchange.is_connected:
            issues.append(DiagIssue(
                ...
                recommendation="API Key و TESTNET را بررسی کنید",
            ))
        ...
        now  = time.time()
        errs = [e for e in self._err_hist if now - e["ts"] < 3600]
        if len(errs) > 20:
            ...
            issues.append(DiagIssue(
                severity="warning", category="system",
                title="خطاهای مکرر: " + str(len(errs)),
                description="نوع: " + top + " (" + str(by[top]) + "×)",
                recommendation="لاگ‌ها را بررسی کنید",
                data={"by_type": dict(by)},
            ))
        return {
    ...
    def _fix_no_trade(self, top) -> str:
        ...
        for r, _ in top:
            ...
            if any(w in rl for w in ["روند","adx","trend"]):
                fixes.append("ADX threshold کاهش دهید")
            elif any(w in rl for w in ["conf","اطمینان"]):
                fixes.append("MIN_CONFIDENCE کاهش دهید")
            elif any(w in rl for w in ["حجم","vol"]):
                fixes.append("آستانه حجم کاهش دهید")
            elif any(w in rl for w in ["داده","data","ohlcv"]):
                fixes.append("/diagnose را بررسی کنید")
        return " | ".join(fixes[:3]) if fixes else "MIN_CONFIDENCE=45 | SCAN_INTERVAL=30"
        ...
        for sym, stats in self._sym_stats.items():
            ...
            if stats["scans"] > 0:
                ...
                if er > 50:
                    ...
                    issues.append(DiagIssue(
                        ...
                        title="خطای بالا: " + sym + " " + str(round(er)) + "%",
                        description="این نماد مشکل دارد",
            ...
            if tc >= 5:
                ...
                if wr < 30:
                    ...
                    issues.append(DiagIssue(
                        severity="critical", category="symbol",
                        title="WR پایین: " + sym + " " + str(round(wr)) + "%",
        ...
        for strat in set(list(self._strat_stats.keys()) + list(db_s.keys())):
            ...
            if total >= 3 and wins / total * 100 < 40:
                ...
                issues.append(DiagIssue(
                    ...
                    title="استراتژی ضعیف: " + strat + " " + str(round(wr)) + "%",
    ...
    def _gen_recs(self, issues, db, engine) -> List[str]:
        ...
        if len(ohlcv_errs) > 10:
            recs.append("مشکل OHLCV - فرمت نماد یا TESTNET را بررسی کنید")
    ...
    def _gen_summary(self, issues, score) -> str:
        ...
        st = ("سیستم سالم" if score >= 80 else
              "نیاز به توجه" if score >= 60 else
              "مشکلات جدی" if score >= 40 else "بحرانی")
    ...
    def fmt_web(self, report: DiagReport) -> str:
        ...
        for i in report.issues:
            ic  = "#f85149" if i.severity=="critical" else (
                  "#f0883e" if i.severity=="warning"  else "#58a6ff")
            ...
            iss += (
                ...
                ';padding:8px;margin:5px 0;background:#0d1117;border-radius:4px;">'
                '<b style="color:' + ic + ';">' + i.title + '</b> ' + fb +
                '<p style="margin:2px 0;font-size:.85em;color:#8b949e;">' +
                i.description.replace("\n","<br>") + '</p>'
                '<p style="margin:2px 0;font-size:.82em;color:#3fb950;">💡 ' +
                i.recommendation.replace("\n","<br>") + '</p>'
                '</div>'
            )
        ...
        for sym, data in report.symbol_health.items():
            ...
            pnlc = "#3fb950" if pnl>=0 else "#f85149"
            er_r = (stats["errors"] / stats["scans"] * 100
                    if stats.get("scans",0) > 0 else 0)
            ...
            sym_rows += (
                ...
                "<td style='color:" + errc + "'>" + str(stats.get("errors",0)) +
                " (" + str(round(er_r)) + "%)</td></tr>"
            )
        ...
        return (
            ...
            '<h2 style="color:#58a6ff;">🧠 AI Report v6.3</h2>'
            '<div style="font-size:3em;font-weight:bold;color:' + sc + ';">' +
            str(s) + '</div>'
            ...
            '<h3>💡 توصیه‌ها</h3>'
            '<ul style="color:#3fb950;margin:0;">' + rli + '</ul>'
            '</div>'
            '<div style="background:#161b22;border-radius:8px;padding:12px;margin:8px 0;">'
            '<h3>🔍 مشکلات (' + str(len(report.issues)) + ')</h3>' +
            (iss if iss else '<p style="color:#3fb950;">✅ مشکل مهمی نیست</p>') +
            '</div>'
            '<div style="background:#161b22;border-radius:8px;padding:12px;margin:8px 0;">'
            '<h3>📈 استراتژی‌ها</h3>'
            '<table style="width:100%;border-collapse:collapse;">'
            '<tr style="color:#58a6ff;"><th>نام</th><th>معاملات</th>'
            '<th>WR</th><th>PnL</th></tr>' +
            (st_rows if st_rows else nd) +
            '</table></div>'
            '<div style="background:#161b22;border-radius:8px;padding:12px;margin:8px 0;">'
            '<h3>🪙 نمادها</h3>'
            '<table style="width:100%;border-collapse:collapse;">'
            '<tr style="color:#58a6ff;"><th>نماد</th><th>اسکن</th>'
            '<th>سیگنال</th><th>معامله</th><th>PnL</th><th>خطا</th></tr>' +
            (sym_rows if sym_rows else ns) +
            '</table></div>'
            '</div>'
        )
...
class Exchange:
    ...
    def _connect(self):
        ...
        try:
            ...
            # ─── حل اصلی: تبدیل هوشمند نمادها ───
            RESOLVER.resolve(markets, SYMBOLS_WANTED)

            self._set_leverage()
            self._connected = True
            log.info("✅ Phemex متصل | %d نماد فعال",
                     len(RESOLVER.active_wanted))

        except ccxt.AuthenticationError as e:
            log.error("❌ خطای Auth: %s", e)
    ...
    def fetch_ohlcv(self, sym: str, tf: str = "5m",
        ...
        for attempt in range(retries):
            try:
                ...
                log.error("❌ نماد نامعتبر: %s (real=%s)", sym, real)
    ...
    def fetch_multi(self, sym: str) -> Dict:
        result = {}
        # تایم‌فریم‌ها بر اساس PRIMARY_TF
        base_tfs = ["1h", "15m", PRIMARY_TF]
        seen = set()
        configs = []
        for tf in base_tfs:
            if tf not in seen:
                seen.add(tf)
                lim = 200 if tf=="1h" else 100 if tf in ("15m","30m") else 80
                configs.append((tf, lim))
        ...
        # Fallback کامل
        if "5m" not in result:
            for candidate in [PRIMARY_TF, "15m", "1h"]:
                if candidate in result:
                    result["5m"] = result[candidate].copy()
                    break
    ...
    def place_order(self, sym: str, side: str,
        ...
        try:
            ...
            if not price:
                return None
            cs        = RESOLVER.get_contract_size(sym)
    ...
    def diagnose(self) -> Dict:
        result = {
            ...
            "symbol_map":     {RESOLVER.base(k): v
                               for k, v in RESOLVER._map.items()},
            "balance_test":   None,
            "ohlcv_test":     {},
        }
        if not self._ex:
...
class StrategyEngine:
    ...
    def _main_df(self, dfs: Dict) -> Optional[pd.DataFrame]:
    ...
    def _breakout(self, sym: str, dfs: Dict) -> Signal:
        ...
        if vr < 1.1:
            DIAG.rec_scan(sym,"no_signal","Breakout: حجم " + str(round(vr,1)) + "x")
            return Signal()
    ...
    def _pullback(self, sym: str, dfs: Dict) -> Signal:
        df = dfs.get("15m") or self._main_df(dfs)
        if df is None or len(df) < 20: return Signal()
        ctx   = self._ctx(dfs)
        c     = df["close"]
        price = IND.safe(c)
        ...
        if ctx["trend"] in ("up","weak_up") and -2.0<dist<0.5 and 40<rsi<70:
            ...
            return Signal("buy","Pullback",min(85,conf),
                          "EMA20 (" + str(round(dist,1)) + "%) RSI=" + str(round(rsi)),
    ...
    def _macd_adx(self, sym: str, dfs: Dict) -> Signal:
        ...
        adx=IND.safe(IND.adx(h,l,c,14))
        if adx < 18:
            DIAG.rec_scan(sym,"no_signal","MACD: ADX=" + str(round(adx)))
            return Signal()
        if mp<sp and mv>sv:
            sl,tp,tp1,atr = self._levels(df,price,"buy")
...
class TG:
    ...
    def _positions(self):
        ...
        for p in real:
            lines.append(
                RESOLVER.base(p["symbol"] or "") + " " + p["side"].upper() +
    ...
    def _status(self):
        ...
        sym_map = {RESOLVER.base(k): v for k,v in RESOLVER._map.items()}
        sym_txt = NL.join(
            "  " + b + " → " + r for b,r in list(sym_map.items())[:5]
        )
        msg = (
            "⚙️ <b>وضعیت v6.3</b>" + NL + "═"*28 + NL +
            ("✅" if EX.is_connected else "❌") +
            " | " + ("TESTNET" if TESTNET else "MAINNET") + NL +
            "💰 $" + str(round(bal,2)) + NL +
            "TF: " + PRIMARY_TF + " | Risk: " + str(RISK_PCT) + "%" + NL +
            "MaxPos:" + str(MAX_POS) + " | Scan:" + str(SCAN_INTERVAL) + "s" + NL +
            "MinConf:" + str(MIN_CONFIDENCE) + "%" + NL +
            "═"*28 + NL +
            "🪙 نگاشت نمادها:" + NL + sym_txt
        )
        self.send(msg, self.kb())
    ...
    def _debug(self):
        ...
        lines = [
            ...
            "📊 " + str(len(self.eng._pos)) + "/" + str(MAX_POS),
            "TF: " + PRIMARY_TF,
            "",
        ]
...
class Engine:
    ...
    def _boot(self):
        ...
        for rp in EX.fetch_positions():
            if not any(p["symbol"]==rp["symbol"] for p in self._pos.values()):
                ...
                e   = rp["entry"]
                cs  = RESOLVER.get_contract_size(rp.get("symbol","BTC/USDT"))
                pos = {
                    ...
                    "trailing_active":False,"atr_value":e*0.01,
                    "highest_pnl_pct":0,
                }
                self._pos[pid] = pos
                database.insert(pos)
    ...
    def run_loop(self):
        log.info("🚀 v6.3 | TF=%s | %d نماد | %s",
                 PRIMARY_TF, len(SYMBOLS), "TESTNET" if TESTNET else "MAINNET")
            ...
            if self.tg:
                if crits:
                    self.tg.send(
                        ...
                        "🧠 تشخیص AI برای جزئیات"
                    )
                else:
                    self.tg.send(
                        "✅ <b>سیستم سالم - v6.3</b>" + NL +
                        "امتیاز: " + str(rep.health_score) + "/100" + NL +
                        "TF: " + PRIMARY_TF + " | " +
                        str(len(SYMBOLS)) + " نماد فعال" + NL +
    ...
    def _auto_diag(self):
        try:
            ...
            if self.tg and rep.health_score < 60:
                NL = "\n"
                self.tg.send(
                    "⚠️ <b>هشدار AI</b>" + NL + rep.summary + NL +
                    "🧠 تشخیص AI برای جزئیات"
                )
        except Exception as e:
            ...
            try:
                ...
                with concurrent.futures.ThreadPoolExecutor() as ex:
                    try:
                        ...
                        DIAG.rec_error(sym,"timeout","Timeout")
                        continue

                if not dfs:
                    DIAG.rec_scan(sym,"no_signal","داده نیست")
                    continue
                ...
                if sig.confidence < MIN_CONFIDENCE:
                    DIAG.rec_scan(sym,"no_signal","Conf=" + str(sig.confidence) + "%")
                    continue
    ...
    def _execute(self, sym: str, sig: Signal, balance: float):
        ...
        pos   = {
            ...
            "trailing_active":False,"atr_value":sig.atr_value,
            "highest_pnl_pct":0,
        }
        with self._lock:
            self._pos[pid] = pos
    ...
    def _manage(self):
        ...
        for pid, pos in snap.items():
            try:
                ...
                # TP
                if ((side=="long" and price>=pos["tp"]) or
    ...
    def _partial(self, pid: str, pos: Dict, price: float):
        ...
        if res:
            ...
            nid = EX.update_sl(pos["symbol"],pos["side"],rq,
                               pos.get("sl_order_id",""),nsl)
            if nid: pos["sl_order_id"] = nid
    ...
    def _close(self, pid: str, pos: Dict, price: float, reason: str):
        ...
        log.info("%s [%s] %s PnL:%+.2f$",
                 icon, RESOLVER.base(pos["symbol"]), reason, pnl)
        if self.tg:
            NL = "\n"
            self.tg.send(
                icon + " <b>" + reason + " | " +
                RESOLVER.base(pos["symbol"]) + "</b>" + NL +
                "PnL: " + str(round(pnl,2)) +
                "$ (" + str(round(pct,2)) + "%)"
            )
...
def home():
    ...
    if engine_instance:
        for pid, pos in engine_instance._pos.items():
            ...
            if price:
                ...
                pos_html += (
                    "<div class='card' style='border-color:" + c + ";min-width:170px;'>"
                    "<b>" + RESOLVER.base(pos["symbol"]) + " " +
    ...
    sym_badges = "".join(
        "<span style='background:#21262d;padding:2px 6px;border-radius:4px;"
        "margin:2px;font-size:.8em;display:inline-block;'>" +
        RESOLVER.base(s) + "</span>"
        for s in SYMBOLS
    )

    return (
        "<!DOCTYPE html><html dir='rtl' lang='fa'><head>"
        "<meta charset='UTF-8'><title>Quant Bot v6.3</title>"
        ...
        "min-width:115px;vertical-align:top}"
        ...
        "</style></head><body>"
        "<h1>🤖 Master-AI Quant Bot v6.3</h1>"
        "<span class='badge'>TF:" + PRIMARY_TF + " | " + mode + " | " +
        str(len(SYMBOLS)) + " نماد</span>"
        "<div class='sec'>"
        + ("▶️ فعال" if act else "⏹ متوقف") +
        " | " + ("✅" if EX.is_connected else "❌") +
        " | " + str(pc) + "/" + str(MAX_POS) +
        "</div>"
        "<div class='sec'>"
        ...
        "</div>"
        "<div class='sec'>"
        "<div class='card' style='min-width:200px;border-color:" + sc + ";'>"
        "<h3>🧠 AI</h3>"
        "<div style='font-size:2.5em;color:" + sc + ";'>" + str(sc_v) + "</div>"
        ...
        "<p>آخرین معامله: " + lh + "</p>"
        "<p><a href='/ai-report'>📋 AI Report</a> | "
        "<a href='/diagnose'>🔍 Diagnose</a></p>"
        "</div></div>"
        "<div class='sec'><h3>📈 پوزیشن‌ها</h3>"
        + (pos_html if pos_html else "<p style='color:#8b949e;'>هیچ پوزیشنی نیست</p>") +
        "</div>"
        "<div class='sec'><h3>🪙 نمادهای فعال</h3>" + sym_badges + "</div>"
        "<div class='sec'>"
        "<a href='/ai-report' class='badge' style='padding:8px 16px;'>🧠 AI Report</a>"
        " <a href='/diagnose' class='badge' style='background:#1f6feb;padding:8px 16px;'>"
        "🔍 Diagnose</a>"
        " <a href='/debug' class='badge' style='background:#6e40c9;padding:8px 16px;'>"
        "🛠️ Debug</a>"
        "</div></body></html>"
    )
...
def diagnose():
    ...
    bl   = ("✅ $" + str(bt.get("free","?")) + " آزاد" if bt.get("ok")
            else "❌ " + str(bt.get("error","?"))[:50])
    ...
    for base, ot in info.get("ohlcv_test",{}).items():
        ...
        rows += (
            ...
            ("✅ " + str(ot.get("candles",0)) + " کندل [" + ot.get("tf","?") + "]"
             if ook else "❌ " + str(ot.get("error",""))[:50]) +
            "</td></tr>"
        )
    ...
    for wanted, real in RESOLVER._map.items():
        ...
        sym_map_rows += (
            "<tr><td>" + wanted + "</td>"
            "<td style='color:" + c + "'>" + real + "</td>"
            "<td>" + ("✅ مطابق" if same else "⚠️ تبدیل‌شده") + "</td></tr>"
        )

    return (
        "<!DOCTYPE html><html dir='rtl' lang='fa'><head>"
        "<meta charset='UTF-8'><title>تشخیص v6.3</title>"
        "<style>"
        "body{font-family:Tahoma;background:#0d1117;color:#c9d1d9;"
        ...
        "<p>🔗 اتصال: <b>" + con + "</b></p>"
        "<p>💰 موجودی: <b>" + bl + "</b></p>"
        "<p>⏱️ TF: <b>" + PRIMARY_TF + "</b></p>"
        "<p>🪙 نمادهای یافت‌شده: <b>" +
        str(info.get("symbols_found",0)) + "/" + str(len(SYMBOLS_WANTED)) + "</b></p>"
        "</div>"
        "<div class='card'>"
        "<h3>📊 تست OHLCV</h3>"
        "<table>"
        "<tr style='color:#58a6ff;'><th>نماد</th><th>نام در صرافی</th><th>نتیجه</th></tr>" +
        (rows if rows else "<tr><td colspan='3'>---</td></tr>") +
        "</table></div>"
        "<div class='card'>"
        "<h3>🔄 نگاشت نمادها</h3>"
        "<table>"
        "<tr style='color:#58a6ff;'><th>خواسته‌شده</th>"
        "<th>واقعی در صرافی</th><th>وضعیت</th></tr>" +
        (sym_map_rows if sym_map_rows else "<tr><td colspan='3'>---</td></tr>") +
        "</table></div>"
        "<div class='card'>"
        "<h3>🛠️ راهنمای رفع مشکل</h3>"
        "<ol>"
        "<li><b>OHLCV ❌:</b> PHEMEX_TESTNET را بررسی کنید (true/false)</li>"
        "<li><b>نماد ⚠️ تبدیل‌شده:</b> طبیعی است - ربات خودکار تطبیق می‌دهد</li>"
        "<li><b>موجودی ❌:</b> کلیدهای API اشتباه یا منقضی هستند</li>"
        "<li><b>اتصال ❌:</b> کلید جدید از Phemex بگیرید</li>"
        "</ol>"
        "</div></body></html>"
    )
...
def ai_report():
    ...
    try:
        ...
        return (
            "<!DOCTYPE html><html dir='rtl' lang='fa'><head>"
            "<meta charset='UTF-8'><title>AI Report v6.3</title>"
            "<style>"
...
def api_debug():
    ...
    for sym in SYMBOLS:
        ...
        try:
            ...
            if not dfs:
                results[sn] = {"error":"no data","real":RESOLVER.real(sym)}
                continue
            sig = STRATEGY.analyze(sym, dfs)
            slp = (round(abs(sig.sl-sig.entry_estimate)/sig.entry_estimate*100,2)
                   if sig.entry_estimate else 0)
            tpp = (round(abs(sig.tp-sig.entry_estimate)/sig.entry_estimate*100,2)
                   if sig.entry_estimate else 0)
            results[sn] = {
                "action":     sig.action,
...
def main():
    ...
    log.info("=" * 60)
    log.info("  🤖 Master-AI Quant Bot v6.3")
    log.info("  🔑 مشکل BTC/USDT vs BTC/USDT:USDT رفع شد")
    ...
    if TG_TOKEN and TG_CHAT:
        ...
        tg.send(
            "🚀 <b>ربات v6.3 شروع شد</b>" + NL + "═"*28 + NL +
            ("🧪 TESTNET" if TESTNET else "💰 MAINNET") + NL +
            "⏱️ TF: " + PRIMARY_TF + NL +
            "🪙 " + sym_list + " ..." + NL +
            "🔗 " + ("✅" if EX.is_connected else "❌") + " اتصال" + NL +
            "═"*28 + NL +
            "دستورات: 🧠 تشخیص AI | 🔍 دیباگ",
            kb=tg.kb(),
        )
