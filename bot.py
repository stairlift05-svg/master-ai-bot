...
# فقط برای فعال‌سازی Trailing استفاده می‌شود — دیگر روی اجرای SL پایه تأثیر ندارد
...
SCAN_INTERVAL = 55
SYMBOL_DELAY = 1.2          # کاهش از 2.5 برای اسکن سریع‌تر
BINANCE_BAN_SECONDS = 200   # کول‌داون Binance اکنون به تفکیک نماد است
...
class StrategyEngine:
    def analyze(self, df_5m: pd.DataFrame, df_1h: pd.DataFrame) -> dict:
        ...
        # فیلتر روند نرم‌تر نسبت به نسخه قبلی (کاهش رد شدن‌های بی‌مورد)
        if hp > e50 * 0.992 and e50 >= e200 * 0.985:
            htf_trend = "bullish"
        elif hp < e50 * 1.008 and e50 <= e200 * 1.015:
            htf_trend = "bearish"
        else:
...
class QuantEngine:
    def __init__(self):
        ...
        self.ex_data = ccxt.binance({"enableRateLimit": True, "options": {"defaultType": "spot"}})
        self.prices: Dict[str, float] = {}
        self.open_times: Dict[str, float] = {}
        # کول‌داون Binance اکنون به تفکیک نماد است (نه سراسری برای همه نمادها)
        self._binance_ban_until: Dict[str, float] = {}

    async def fetch_ohlcv(self, symbol: str, timeframe: str, limit: int = 100) -> list:
        now = time.time()
        if now < self._binance_ban_until.get(symbol, 0):
            return []
        ...
        try:
            ...
            if "418" in err or "banned" in err.lower() or "-1003" in err:
                self._binance_ban_until[symbol] = now + BINANCE_BAN_SECONDS
                log.error(f"Binance rate-limit/ban برای {symbol} – کول‌داون {BINANCE_BAN_SECONDS}s")
                await self.db.log_event(symbol, "binance_ban", False,
                                         "محدودیت/بن Binance – کول‌داون فقط برای این نماد فعال شد", extra=err[:150])
            else:
                await self.db.log_event(symbol, "binance_fetch_error", False, err[:150])
            await self.db.log_decision(symbol, "neutral", "", "خطا در دریافت کندل Binance", extra=err[:150])
            return []

    async def start(self):
        await self.db.init()
        log.info("v14.3 Hybrid Tuned + Forensic Report starting...")
        try:
            ...
            for sym in SYMBOLS:
                ...
                with STATE_LOCK:
                    if any(p["symbol"] == sym for p in SHARED_STATE["active_positions"].values()):
                        continue
                try:
                    # واکشی موازی کندل ۵m و ۱h برای کاهش تاخیر چرخه اسکن
                    ...
                    await self.db.log_event(sym, "scan_error", False, str(e)[:150])
                await asyncio.sleep(SYMBOL_DELAY)
    ...
    async def execute_trade(self, sym: str, sig: dict):
        ...
        try:
            ...
            if qty <= 0:
                ...
                await self.db.log_event(sym, "order_open", False, "حجم محاسبه‌شده صفر بود - سفارش ارسال نشد", price)
                return
            ...
            await self.db.log_event(sym, "order_open", True,
                                     f"ورود {sig['action'].upper()} با استراتژی {sig['strat']}",
                                     price=fill, qty=qty,
                                     extra=f"SL={sig['sl']:.5f} TP={sig['tp']:.5f} TP1={sig['tp1']:.5f} RR={sig.get('expected_rr','?')}")
            SYMBOL_ERROR_COUNT.pop(sym, None)
            SYMBOL_ERROR_COOLDOWN.pop(sym, None)
            await self.tg.send(
                f"🎯 <b>ورود {sig['action'].upper()}</b> ({sig['strat']})\n"
                f"{sym} @ {fill:.4f}\nRR≈{sig.get('expected_rr', '?')} | MinHold(Trail) {MIN_HOLD_SECONDS//60}m")
        except Exception as e:
            err = str(e)
            ...
            await self.db.log_event(sym, "order_open", False, f"خطا در ارسال سفارش ورود: {err[:150]}", price)
    ...
    async def real_test_trade(self):
        ...
        try:
            ...
            with STATE_LOCK:
                SHARED_STATE["active_positions"][pid] = pos
            self.open_times[pid] = time.time()
            await self.db.log_event(TEST_SYMBOL, "real_test_open", True, "پوزیشن تست واقعی باز شد", price=fill, qty=qty)
            ...
            await self.db.log_event(TEST_SYMBOL, "real_test_open", False, f"خطا در تست واقعی: {str(e)[:150]}")
        ...
        try:
            ...
            for pid, pos in local_items:
                ...
                if pos["symbol"] not in remote_map:
                    ...
                    await self.db.log_event(pos["symbol"], "sync_close_missing", True,
                                             "پوزیشن محلی روی صرافی پیدا نشد و بسته ثبت شد", price=pos.get("entry", 0))
                    with STATE_LOCK:
            ...
            for sym, rpos in remote_map.items():
                ...
                # تلاش برای محاسبه ATR واقعی نماد به‌جای ضریب ثابت
                atr_a = entry * 0.015
                try:
                    raw5 = await self.fetch_ohlcv(sym, TIMEFRAME, 60)
                    if raw5 and len(raw5) >= 20:
                        df_tmp = pd.DataFrame(raw5, columns=["ts", "open", "high", "low", "close", "volume"])
                        real_atr = float(Indicators.atr(df_tmp, 14).iloc[-1])
                        if real_atr > 0:
                            atr_a = real_atr
                except Exception:
                    pass
                ...
                self.open_times[pid] = time.time()
                await self.db.insert_trade(pos)
                await self.db.log_event(sym, "sync_recovered", True,
                                         "پوزیشن روی صرافی پیدا شد و در ربات بازیابی شد",
                                         price=entry, qty=rpos["qty"])
            ...
            await self.db.log_event("ALL", "sync_error", False, str(e)[:150])
    ...
    async def force_close(self, pid: str, reason: str):
        ...
        try:
            ...
            await self.db.log_event(pos["symbol"], "order_close", True,
                                     f"پوزیشن بسته شد - دلیل: {reason}",
                                     price=price, qty=pos["qty"], extra=f"PnL={net:+.3f} Hold={hold/60:.1f}m")
            ...
            if net < 0:
                with STATE_LOCK:
                    ...
                    if cl[sym]["count"] >= CONSECUTIVE_LOSS_LIMIT:
                        ...
                        await self.db.log_event(sym, "cooldown_triggered", True,
                                                 f"{cl[sym]['count']} ضرر متوالی → کول‌داون {SYMBOL_COOLDOWN_HOURS} ساعته")
            ...
            await self.db.log_event(pos["symbol"], "order_close", False, f"خطا در بستن پوزیشن: {str(e)[:150]}", price)
    ...
    async def watchdog_loop(self):
        while True:
            ...
            for pid, pos in items:
                ...
                # min-hold فقط تصمیم می‌گیرد که آیا Trailing فعال شود یا نه — SL پایه همیشه فعال است
                can_trail = hold >= MIN_HOLD_SECONDS
                ...
                if can_trail and pnl_pct > TRAIL_ACT and pnl_pct > pos["highest_pnl_pct"]:
                    ...
                    if (pos["side"] == "buy" and new_sl > pos["sl"]) or (pos["side"] == "sell" and new_sl < pos["sl"]):
                        ...
                        await self.db.log_event(pos["symbol"], "trailing_update", True,
                                                 f"Trailing SL به‌روزرسانی شد (PnL={pnl_pct:.2f}%)", price=price)

                # Partial TP (از همان ابتدا فعال است)
                    ...
                    if hit:
                        try:
                            ...
                            if half > 0:
                                ...
                                pos["qty"] -= half
                                pos["is_partial"] = 1
                                # Break-even بلافاصله فعال می‌شود — بدون وابستگی به min-hold
                                pos["sl"] = pos["entry"]
                                await self.db.update_trade(pid, pos["qty"], pos["sl"], 1, pos["highest_pnl_pct"])
                                await self.db.log_event(pos["symbol"], "partial_tp", True,
                                                         "نیمی از پوزیشن بسته شد و SL به Break-even منتقل شد",
                                                         price=price, qty=half)
                                await self.tg.send(f"🔹 Partial TP → BE {pos['symbol']}")
                        except Exception as e:
                            await self.db.log_event(pos["symbol"], "partial_tp", False, f"خطا در Partial TP: {str(e)[:120]}", price)

                # SL پایه از همان لحظه ورود فعال است | TP همیشه فعال است
