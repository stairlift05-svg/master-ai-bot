...
Master Quant Engine v17.0 – QQE Strategy (Phemex)

- استراتژی QQE (Qualitative Quantitative Estimation) بهینه‌شده

- ۳ ساب‌استراتژی QQE

- فیلتر RSI 50

- خروج با سیگنال مخالف QQE

- تایم‌فریم 1h + HTF 4h (بک‌تست +33%)

- تنظیمات بهینه‌شده برای کریپتو
...
# 🎯 QQE STRATEGY PARAMETERS (Optimized for Crypto)
# ═══════════════════════════════════════════════════════════════

QQE_PARAMS = {

    "rsi_period":   20,      # RSI Length (بهینه‌شده برای 1h: 20)

    "sf":           7,       # RSI Smoothing (بهینه‌شده برای 1h: 7)

    "qqe_factor":   6.0,     # Fast QQE Factor (بهینه‌شده: 6.0 باندهای عریض‌تر)

    "threshold":    10,      # Threshold Channel

    "use_rsi50":    True,    # فیلتر RSI 50 (ضروری!)
    "use_ema200":   True,    # فیلتر EMA 200 (برای 1h ضروری است)

    "use_htf_qqe":  True,    # فیلتر QQE روی HTF

}

STRATEGY_PARAMS = {

    "QQE_Cross":        {"sl_m": 2.5, "tp_m": 5.0, "tp1_m": 2.5},   # 2:1 RR – اصلی

    "QQE_Momentum":     {"sl_m": 2.5, "tp_m": 6.25, "tp1_m": 2.5},  # 2.5:1 RR – با تایید حجم

    "QQE_Conservative": {"sl_m": 2.5, "tp_m": 7.5, "tp1_m": 2.5},   # 3:1 RR – با HTF QQE

}

TIMEFRAME = "1h"                  # بهترین تایم‌فریم پایین برای QQE (بک‌تست +33%)

HTF_TIMEFRAME = "4h"             # تایم‌فریم بالاتر برای فیلتر روند
...
MIN_HOLD_FOR_PARTIAL = 3600          # 1 ساعت (با تایم‌فریم 1h)

MIN_HOLD_FOR_TRAIL = 5400            # 1.5 ساعت
...
POST_CLOSE_COOLDOWN = 1800           # 30 دقیقه

SCAN_INTERVAL = 45                   # هر 45 ثانیه

SYMBOL_DELAY = 1.3

EXIT_ON_OPPOSITE_QQE = True          # خروج با سیگنال مخالف QQE
...
SHARED_STATE: Dict[str, Any] = {
    ...
    "qqe_signals": {},                # آخرین سیگنال QQE هر نماد
...
class Database:
    ...
    async def generate_txt_report(self, prices: Dict[str, float] = None) -> str:
        ...
        lines = [
            ...
            "       MASTER QUANT ENGINE v17.0 – QQE Strategy REPORT",
            ...
            f"│  RSI Period     : {QQE_PARAMS['rsi_period']}",

            f"│  RSI Smoothing  : {QQE_PARAMS['sf']}",

            f"│  QQE Factor     : {QQE_PARAMS['qqe_factor']}",

            f"│  Timeframe      : {TIMEFRAME} / HTF {HTF_TIMEFRAME}",

            f"│  RSI 50 Filter  : {QQE_PARAMS['use_rsi50']}",

            f"│  HTF QQE Filter : {QQE_PARAMS['use_htf_qqe']}",

            f"│  Exit Opposite  : {EXIT_ON_OPPOSITE_QQE}",
        ...
        lines.append("┌─ 5. LAST QQE SIGNALS ──────────────────────────────────────────────")

        qqe_sigs = st.get("qqe_signals", {})

        if not qqe_sigs:

            lines.append("│  (no QQE signals yet)")

        else:

            for sym, sig in qqe_sigs.items():

                lines.append(f"│  {sym:<18} {sig.get('action',''):>6} | {sig.get('reason','')[:40]}")
...
class Indicators:
    ...
    def calculate_qqe(df: pd.DataFrame, rsi_period: int = 6, sf: int = 3,

                      qqe_factor: float = 5.0, threshold: int = 10) -> dict:

        """

        Calculate QQE (Qualitative Quantitative Estimation) indicator.

        

        Returns dict with:

            rsi_ma: Smoothed RSI (خط نارنجی)

            fast_atr_rsi_tl: Fast ATR RSI Trailing Level (خط آبی)

            trend: Current trend direction (1=bullish, -1=bearish)

            long_signal: QQE bullish cross signal (دایره سبز)

            short_signal: QQE bearish cross signal (دایره آبی)

            above_threshold: RSI above 50+threshold

            below_threshold: RSI below 50-threshold
        ...
        ma_atr_rsi = Indicators.ema(atr_rsi, wilders_period)

        dar = Indicators.ema(ma_atr_rsi, wilders_period) * qqe_factor
        ...
        fast_atr_rsi_tl = pd.Series(0.0, index=df.index)

        # Initialize first values

        longband.iloc[0] = newlongband.iloc[0] if not pd.isna(newlongband.iloc[0]) else 0

        shortband.iloc[0] = newshortband.iloc[0] if not pd.isna(newshortband.iloc[0]) else 0

        trend.iloc[0] = 1

        fast_atr_rsi_tl.iloc[0] = longband.iloc[0]

        for i in range(1, n):

            # Skip if NaN values

            if pd.isna(rsi_ma.iloc[i]) or pd.isna(rsi_ma.iloc[i-1]):

                trend.iloc[i] = trend.iloc[i-1]

                fast_atr_rsi_tl.iloc[i] = fast_atr_rsi_tl.iloc[i-1]

                continue

            # Longband (trailing stop logic)
            ...
            # Trend detection (cross detection)

            # cross(RSIndex, shortband[1]) → RSIndex crosses above shortband[1]

            if rsi_ma.iloc[i] > shortband.iloc[i-1] and rsi_ma.iloc[i-1] <= shortband.iloc[i-1]:

                trend.iloc[i] = 1

            # cross(longband[1], RSIndex) → RSIndex crosses below longband[1]

            elif rsi_ma.iloc[i] < longband.iloc[i-1] and rsi_ma.iloc[i-1] >= longband.iloc[i-1]:
            ...
            # Fast ATR RSI Trailing Level

            fast_atr_rsi_tl.iloc[i] = longband.iloc[i] if trend.iloc[i] == 1 else shortband.iloc[i]

        # ── QQE Cross Signals ──

        # 🟢 Long Signal: خط نارنجی خط آبی را رو به بالا قطع کند

        #   FastAtrRsiTL[1] >= RSIndex[1] and FastAtrRsiTL < RSIndex

        long_signal = (

            (fast_atr_rsi_tl.shift(1) >= rsi_ma.shift(1)) &

            (fast_atr_rsi_tl < rsi_ma)

        )

        # 🔵 Short Signal: خط آبی خط نارنجی را رو به بالا قطع کند
        ...
        above_threshold = rsi_ma > (50 + threshold)

        below_threshold = rsi_ma < (50 - threshold)

        return {

            "rsi_ma": rsi_ma,

            "fast_atr_rsi_tl": fast_atr_rsi_tl,
            ...
            "rsi_ma_last": float(rsi_ma.iloc[-1]) if not pd.isna(rsi_ma.iloc[-1]) else 50.0,

            "fast_atr_rsi_tl_last": float(fast_atr_rsi_tl.iloc[-1]) if not pd.isna(fast_atr_rsi_tl.iloc[-1]) else 50.0,

            "trend_last": int(trend.iloc[-1]),

            "long_signal_last": bool(long_signal.iloc[-1]),

            "short_signal_last": bool(short_signal.iloc[-1]),

            "above_threshold_last": bool(above_threshold.iloc[-1]),

            "below_threshold_last": bool(below_threshold.iloc[-1]),
...
class StrategyEngine:
    ...
    QQE Strategy Engine - 3 Sub-strategies

    

    1. QQE_Cross:        سیگنال کراس QQE + فیلتر RSI 50 (اصلی)

    2. QQE_Momentum:     سیگنال کراس QQE + RSI 50 + تایید حجم (با کیفیت‌تر)

    3. QQE_Conservative: سیگنال کراس QQE + RSI 50 + HTF QQE ترند (محافظه‌کار)

    """

    def analyze(self, df_ltf: pd.DataFrame, df_htf: pd.DataFrame) -> dict:

        """

        Analyze market data using QQE strategy.

        

        Args:

            df_ltf: Lower timeframe data (15m)

            df_htf: Higher timeframe data (4h)

        

        Returns:

            Signal dict with action, strategy, SL/TP, etc.

        """

        # Use confirmed candles (exclude current forming candle)

        df = df_ltf.iloc[:-1].copy()

        htf = df_htf.iloc[:-1].copy()
        ...
        # ── Calculate QQE on LTF ──

        qqe = Indicators.calculate_qqe(

            df,

            rsi_period=QQE_PARAMS["rsi_period"],

            sf=QQE_PARAMS["sf"],

            qqe_factor=QQE_PARAMS["qqe_factor"],

            threshold=QQE_PARAMS["threshold"],

        )

        if qqe is None:

            return {"action": "neutral", "reason": "خطای محاسبه QQE", "strat": "", "rsi": 0, "atr": 0, "htf": ""}
        ...
        if atr <= 0 or pd.isna(atr):
            ...
            return {"action": "neutral", "reason": "ATR صفر یا نامعتبر", "strat": "", "rsi": 0, "atr": 0, "htf": ""}
        ...
        if atr < atr_sma * 0.32 or atr > atr_sma * 3.8:
            ...
            return {"action": "neutral", "reason": "نوسان نامناسب", "strat": "", "rsi": 0, "atr": atr, "htf": ""}
        ...
        vol_surge = vcur > vsma * 1.45

        # ── RSI ──

        rsi_s = Indicators.rsi(df["close"], QQE_PARAMS["rsi_period"])

        rsi = float(rsi_s.iloc[-1])

        if rsi >= 98 or rsi <= 2 or pd.isna(rsi):

            return {"action": "neutral", "reason": f"RSI نامعتبر ({rsi:.1f})", "strat": "", "rsi": rsi, "atr": atr, "htf": ""}

        # ── QQE Signal Check ──

        qqe_long = qqe["long_signal_last"]

        qqe_short = qqe["short_signal_last"]

        if not qqe_long and not qqe_short:

            return {

                "action": "neutral",

                "reason": f"بدون کراس QQE (trend={'BULL' if qqe['trend_last']==1 else 'BEAR'} RSI={rsi:.1f})",

                "strat": "", "rsi": rsi, "atr": atr, "htf": "bullish" if qqe["trend_last"]==1 else "bearish",

            }

        # ── RSI 50 Filter ──

        rsi_ma = qqe["rsi_ma_last"]

        if QQE_PARAMS["use_rsi50"]:

            if qqe_long and rsi_ma < 50:

                return {"action": "neutral", "reason": f"RSI<50 فیلتر لانگ ({rsi_ma:.1f})", "strat": "", "rsi": rsi, "atr": atr, "htf": ""}

            if qqe_short and rsi_ma > 50:

                return {"action": "neutral", "reason": f"RSI>50 فیلتر شورت ({rsi_ma:.1f})", "strat": "", "rsi": rsi, "atr": atr, "htf": ""}

        # ── EMA 200 Trend Filter (for 1h) ──
        ema200 = float(df["close"].ewm(span=200, adjust=False).mean().iloc[-1])
        if QQE_PARAMS.get("use_ema200", False):
            if qqe_long and price < ema200:
                return {"action": "neutral", "reason": f"EMA200 فیلتر لانگ (price < {ema200:.1f})", "strat": "", "rsi": rsi, "atr": atr, "htf": ""}
            if qqe_short and price > ema200:
                return {"action": "neutral", "reason": f"EMA200 فیلتر شورت (price > {ema200:.1f})", "strat": "", "rsi": rsi, "atr": atr, "htf": ""}

        # ── Calculate HTF QQE ─

        htf_qqe_trend = 0

        if QQE_PARAMS["use_htf_qqe"] and len(htf) >= 20:

            htf_qqe = Indicators.calculate_qqe(
            ...
            if htf_qqe is not None:

                htf_qqe_trend = htf_qqe["trend_last"]

        # ═══════════════════════════════════════════════════════════

        # 🎯 STRATEGY 1: QQE_Cross (اصلی)

        # سیگنال کراس QQE + فیلتر RSI 50

        # ═══════════════════════════════════════════════════════════

        if qqe_long and rsi_ma > 50:

            return self._build("buy", "QQE_Cross", price, atr, rsi, "bullish" if htf_qqe_trend==1 else "neutral")

        if qqe_short and rsi_ma < 50:

            return self._build("sell", "QQE_Cross", price, atr, rsi, "bearish" if htf_qqe_trend==-1 else "neutral")

        # ═══════════════════════════════════════════════════════════

        # 🎯 STRATEGY 2: QQE_Momentum

        # سیگنال کراس QQE + RSI 50 + تایید حجم

        # ═══════════════════════════════════════════════════════════

        if qqe_long and rsi_ma > 50 and vol_surge and 45 < rsi < 70:

            return self._build("buy", "QQE_Momentum", price, atr, rsi, "bullish" if htf_qqe_trend==1 else "neutral")

        if qqe_short and rsi_ma < 50 and vol_surge and 30 < rsi < 55:

            return self._build("sell", "QQE_Momentum", price, atr, rsi, "bearish" if htf_qqe_trend==-1 else "neutral")

        # ═══════════════════════════════════════════════════════════

        # 🎯 STRATEGY 3: QQE_Conservative

        # سیگنال کراس QQE + RSI 50 + HTF QQE ترند

        # ═══════════════════════════════════════════════════════════

        if QQE_PARAMS["use_htf_qqe"] and htf_qqe_trend != 0:

            if qqe_long and rsi_ma > 50 and htf_qqe_trend == 1:

                return self._build("buy", "QQE_Conservative", price, atr, rsi, "bullish")

            if qqe_short and rsi_ma < 50 and htf_qqe_trend == -1:

                return self._build("sell", "QQE_Conservative", price, atr, rsi, "bearish")

        # ── No signal matched ──

        side = "LONG" if qqe_long else "SHORT"

        return {

            "action": "neutral",

            "reason": f"کراس QQE {side} فیلتر شد (RSI={rsi:.1f} Vol={vol_ok} HTF={htf_qqe_trend})",

            "strat": "", "rsi": rsi, "atr": atr,

            "htf": "bullish" if htf_qqe_trend==1 else "bearish" if htf_qqe_trend==-1 else "neutral",

        }

    def _build(self, side, strat, price, atr, rsi, htf):

        p = STRATEGY_PARAMS.get(strat, {"sl_m": 1.0, "tp_m": 2.0, "tp1_m": 1.0})
    ...
    def check_opposite_qqe(self, df_ltf: pd.DataFrame) -> Optional[str]:

        """

        Check if QQE has generated an opposite signal for exit.
        ...
        if qqe["long_signal_last"]:

            return "buy"

        if qqe["short_signal_last"]:
...
class TelegramController:
    ...
    async def poll(self):
        ...
        await self.send(
            ...
            "🚀 <b>Master Quant v17.0 (QQE Strategy)</b>\n"

            f"RSI={QQE_PARAMS['rsi_period']} SF={QQE_PARAMS['sf']} QQE={QQE_PARAMS['qqe_factor']}\n"

            f"TF={TIMEFRAME} HTF={HTF_TIMEFRAME}",
                            ...
                            if d == "cmd_start":
                                ...
                                if not pos:
                                    ...
                                    for p in pos.values():
                                        ...
                                        await self.send(f"{'🟢' if pnl >= 0 else '🔴'} {p['symbol']} | {p['side'].upper()} | ${pnl:.2f} | {p.get('strategy','')}")
                                ...
                                await self.send_document("report.txt", "📄 Report v17.0 QQE")
...
class QuantEngine:
    ...
    async def start(self):
        ...
        log.info("v17.0 QQE Strategy starting...")

        log.info(f"QQE Params: RSI={QQE_PARAMS['rsi_period']} SF={QQE_PARAMS['sf']} "

                 f"QQE={QQE_PARAMS['qqe_factor']} TF={TIMEFRAME} HTF={HTF_TIMEFRAME}")
    ...
    async def scan_loop(self):
        ...
        Main scanning loop - checks for QQE signals and manages positions.

        Also checks for opposite QQE signals on open positions.
        ...
        while True:
            ...
            with STATE_LOCK:
                ...
                for pid, pos in SHARED_STATE["active_positions"].items():

                    if pos["strategy"] != "RealTest":

                        open_syms[pos["symbol"]] = {"pid": pid, "side": pos["side"]}

                can_new = can and len(SHARED_STATE["active_positions"]) < MAX_POS
            ...
            for sym in SYMBOLS:
                ...
                # Skip if in post-close cooldown
                ...
                try:
                    ...
                    raw_ltf, raw_htf = await asyncio.gather(
                    ...
                    if not raw_ltf or len(raw_ltf) < 50:

                        continue

                    df_ltf = pd.DataFrame(raw_ltf, columns=["ts", "open", "high", "low", "close", "volume"])

                    df_htf = (

                        pd.DataFrame(raw_htf, columns=["ts", "open", "high", "low", "close", "volume"])

                        if raw_htf and len(raw_htf) > 30 else df_ltf

                    )

                    price = self.prices.get(sym) or float(df_ltf["close"].iloc[-1])

                    if price <= 0:

                        continue

                    # ── Check for opposite QQE signal on open positions ──

                    if sym in open_syms and EXIT_ON_OPPOSITE_QQE:

                        opp_signal = self.strategy.check_opposite_qqe(df_ltf)

                        pos_info = open_syms[sym]

                        if opp_signal is not None and opp_signal != pos_info["side"]:

                            # Close position on opposite QQE signal

                            log.info(f"🔴 QQE Opposite signal for {sym} (pos: {pos_info['side']}, signal: {opp_signal})")

                            await self.force_close(pos_info["pid"], "QQE_Opposite")

                            await self.tg.send(

                                f"🔄 <b>QQE Opposite</b>\n{sym} | بسته شد ({pos_info['side'].upper()} → {opp_signal.upper()})"
                            ...
                            with STATE_LOCK:
                                ...
                                SHARED_STATE["qqe_signals"][sym] = {

                                    "action": opp_signal,

                                    "reason": "QQE Opposite Exit",

                                    "time": time.strftime("%H:%M:%S"),

                                }

                            # Don't open new position immediately after opposite signal

                            SYMBOL_POST_CLOSE_COOLDOWN[sym] = time.time() + POST_CLOSE_COOLDOWN
                    ...
                    sig = self.strategy.analyze(df_ltf, df_htf)
                    ...
                    if sig["action"] != "neutral":
                        ...
                        if atr > 0:
                            ...
                            p = STRATEGY_PARAMS.get(sig.get("strat", ""), {"sl_m": 1.0, "tp_m": 2.0, "tp1_m": 1.0})
    ...
    async def execute_trade(self, sym: str, sig: dict):
        ...
        try:
            ...
            sl_dist_pct = abs(fill - sig["sl"]) / fill * 100

            tp_dist_pct = abs(sig["tp"] - fill) / fill * 100
            ...
            await self.tg.send(
                ...
                f"SL: {sl_dist_pct:.2f}% | TP: {tp_dist_pct:.2f}%\n"

                f"RR≈{sig.get('expected_rr','?')} | RSI={sig.get('rsi',0):.1f}"
        ...
        try:
            ...
            for sym, rpos in remote_map.items():
                ...
                if rpos["side"] == "buy":
                    ...
                    sl, tp, tp1 = entry - atr * 2.5, entry + atr * 5.0, entry + atr * 2.5
    ...
    async def watchdog_loop(self):
        ...
        Watchdog loop - manages SL/TP, trailing stops, and partial closes.
...
.label{font-size:0.8rem;color:#8b949e;margin-bottom:4px}

.qqe-params{background:#161b22;border:1px solid #30363d;border-radius:10px;padding:14px;margin:20px 0}

.qqe-params h3{color:#f0883e;margin:0 0 8px 0}

.qqe-params table{width:100%;border-collapse:collapse}

.qqe-params td{padding:4px 8px;border-bottom:1px solid #30363d}

.qqe-params td:last-child{text-align:left;color:#58a6ff}
...
<h1>🚀 Master Quant v17.0 – QQE Strategy</h1>

<div class="grid">

<div class="card"><div class="label">موجودی</div><div class="value" id="bal">$0.00</div></div>

<div class="card"><div class="label">پوزیشن</div><div class="value" id="pos">0</div></div>

<div class="card"><div class="label">PnL</div><div class="value" id="pnl">$0.00</div></div>

<div class="card"><div class="label">Win Rate</div><div class="value" id="wr">0%</div></div>

<div class="card"><div class="label">Drawdown</div><div class="value" id="dd">0%</div></div>

</div>

<div class="qqe-params">

<h3>🎯 QQE Parameters</h3>

<table>

<tr><td>RSI Period</td><td>""" + str(QQE_PARAMS["rsi_period"]) + """</td></tr>

<tr><td>RSI Smoothing</td><td>""" + str(QQE_PARAMS["sf"]) + """</td></tr>

<tr><td>QQE Factor</td><td>""" + str(QQE_PARAMS["qqe_factor"]) + """</td></tr>

<tr><td>Timeframe</td><td>""" + TIMEFRAME + """</td></tr>

<tr><td>HTF Timeframe</td><td>""" + HTF_TIMEFRAME + """</td></tr>

<tr><td>RSI 50 Filter</td><td>✅</td></tr>

<tr><td>Exit Opposite</td><td>✅</td></tr>
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("=== Master Quant v17.0 (QQE Strategy) starting ===", flush=True)
