...
# -*- coding: utf-8 -*-
"""
Master Quant Engine v13.7
- رفع حلقه خطای XRP با Cooldown + Exponential Backoff
- تنظیم آستانه اختلاف قیمت جداگانه برای هر نماد
- محدودیت ضرر متوالی (Consecutive Loss Limit)
- بهینه‌سازی نسبت Risk/Reward استراتژی‌ها
- گزارش TXT
"""
...
SYMBOL_CONFIG = {
    ...
    "DOT/USDT:USDT": {"max_price_diff": 3.5,  "min_vol_mult": 1.2},  # DOT نقدینگی پایین‌تر
}

# پارامترهای استراتژی (بهینه‌شده برای RR بهتر)
STRATEGY_PARAMS = {
...
log = logging.getLogger("QuantV13.7")
...
SHARED_STATE: Dict[str, Any] = {
    ...
    "consecutive_losses": {},          # {symbol: {"count": int, "last_loss": timestamp}}
}
STATE_LOCK = Lock()

# Cooldown سراسری برای خطاها
...
class Database:
    ...
    async def generate_txt_report(self) -> str:
        ...
        lines = []
        lines.append("=" * 60)
        lines.append("      MASTER QUANT ENGINE v13.7 - REPORT")
        ...
        if open_trades:
            ...
            for t in open_trades:
                lines.append(f"   {t['symbol']} | {t['side'].upper()} | Entry: {t['entry_price']:.5f}")
            lines.append("")
...
class TelegramController:
    ...
    async def poll(self):
        if not TG_TOKEN: return
        await self.send("🚀 <b>Master Quant v13.7 Online</b>\nCooldown + Consecutive Loss Limit + Better RR", self.menu())
        while True:
            try:
                async with aiohttp.ClientSession() as s:
                        ...
                        for u in data.get("result", []):
                            ...
                            if d == "cmd_start":
                                ...
                                with open("quant_report.txt", "w", encoding="utf-8") as f:
                                    f.write(report)
                                await self.send_document("quant_report.txt", "📄 گزارش کامل TXT v13.7")
            except Exception as e:
...
class QuantEngine:
        ...
        while True:
            ...
            for sym in SYMBOLS:
                ...
                try:
                    ...
                    # آستانه اختلاف قیمت مخصوص هر نماد
                    ...
                    if sig["action"] != "neutral":
                        # تنظیم SL/TP با قیمت واقعی Phemex
                        atr = sig.get("atr", 0)
                        if atr > 0:
    ...
    async def execute_trade(self, sym: str, sig: dict):
        ...
        try:
            ...
            # Exponential Backoff
            cooldown = min(300 * (2 ** (count - 1)), 3600)  # حداکثر ۱ ساعت
